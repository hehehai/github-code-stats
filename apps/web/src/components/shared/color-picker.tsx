...
function ColorPicker(props: ColorPickerProps) {
  ...
  // Initialize listeners set lazily
  const listenersRef = React.useRef<Set<() => void> | null>(null);
  if (!listenersRef.current) {
    listenersRef.current = new Set<() => void>();
  }
...
function ColorPickerImpl(props: ColorPickerImplProps) {
  ...
  // Use prop directly with default instead of useDirection
  ...
  // Callback ref to handle both external ref and local state
  ...
  return (
    <ColorPickerContext.Provider value={contextValue}>
      <Popover
        defaultOpen={defaultOpen}
        modal={modal}
        onOpenChange={(openValue) => store.setOpen(openValue)}
        open={open}
      >
        <div {...rootProps} ref={handleRef} />
        {isFormControl && (
...
function ColorPickerArea(props: React.ComponentProps<"div">) {
  const {
  ...
  // Callback ref to handle both external ref and local ref
  const handleRef = React.useCallback(
...
function ColorPickerHueSlider(
  props: Omit<Slider.Root.Props, "value" | "onValueChange">
) {
...
function ColorPickerSwatch(props: React.ComponentProps<"div">) {
